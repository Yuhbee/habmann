...
void Spiel::enterState(SpielState newState)
    {
        ...
        switch (newState)
        {
            case SpielState::StartState:
                {
                    ...
                    //Da manche Entscheidungen sich unterscheiden, je nachdem welches Spiel gespielt wird, muss der Modus hier festgehalten werden
                    //sollen neben Tic Tac Toe noch weitere Spiele eingefuehrt werden, koennte auch hier ein Enum Sinn machen
                    ticTacToeModus = false;
                    ...
                    //Fuer diese Ausbaustufe kommt noch ein Sonderwert dazu, der anzeigt, dass der Spieler lieber eine Runde Tic Tac Toe spielen moechte
                    vector<int> gueltigeWerte = { 1, 2, 3, 4, 5, 42 };
                    ...
                    //Bei Tic Tac Toe weht kein Wind
                    if (!ticTacToeModus)
                        {
                            windAktiv = false;
            ...
            //Ab hier folgen die neuen Tic Tac Toe States
            case SpielState::SpielerMachtTTTZug:
                {
                    spielfeldanzeigeAktualisierenTTT();

                    //Der Code dieses States aehnelt stark dem des urspruenglichen SpielerMachtZug, aber es ist uebersichtlicher,
                    //einen weiteren State anzulagen, anstatt die Unterschiede im bestehenden Case mit einer Weiche zu umschliessen um das
                    //Tic Tac Toe verhalten umzusetzen
                    bool feldGueltig = false;
                    while (!feldGueltig)
                        {
                            //Hier koennte man eine seperate Meldung einfuehren, aber die vorhandene enthaelt alle notwendigen Informationen
                            spielDarstellung.schussAufforderrung(menschSpieler.getZielfeld().getSpielfeldGroesse());
                            Feld tempFeld = spielEingabe.eingabeFeldKoordinaten();
                            feldGueltig = spielLogik.pruefungEingabe(tempFeld, menschSpieler.getSpielfeld(), Feld::FeldStatus::Leer);
                            if (feldGueltig)
                                {
                                    spielLogik.aendereFeldstatus(tempFeld, menschSpieler.getSpielfeld(), Feld::FeldStatus::Treffer);
                                    spielLogik.aendereFeldstatus(tempFeld, computerSpieler.getSpielfeld(), Feld::FeldStatus::Treffer);
                                }
                        }
                }
            ...
            case SpielState::ComputerMachtTTTZug:
                spielLogik.zufaelligeTTTpositionierung(computerSpieler, menschSpieler);
                break;
        }
...
void Spiel::checkTransition()
    {
        ...
        switch (currentState)
            {
                case SpielState::StartState:
                    //Jetzt gibt es an dieser Stelle eine Pruefung, je nachdem welcher Modus aktiv ist, wird ein anderer State
                    //betreten, nachdem die Initialisierung abgeschlossen ist
                    if (!ticTacToeModus)
                        {
                        ...
                        {
                            enterState(SpielState::SpielerMachtTTTZug);
                        }
                    break;
                case SpielState::SpielerPlatziertSchiffe:
                        {
                ...
                case SpielState::SpielerMachtTTTZug:
                        {
                            switch (spielLogik.spielendePruefenTTT(menschSpieler, computerSpieler))
                                {
                                    case 0:
                                        //Ein Unentschieden gibt es bei Tic Tac Toe, 
                                        spielfeldanzeigeAktualisierenTTT();
                                        spielDarstellung.unentschiedenMeldungAnzeigen();
                                        enterState(SpielState::SpielEnde);
                                        break;
                                    case 1:
                                        //Wurden 3 X in einer Reihe/Spalte/Diagonale platziert gewinnt der Spieler und das Spiel endet
                                        spielfeldanzeigeAktualisierenTTT();
                                        spielDarstellung.gratulationsMeldungAnzeigen();
                                        enterState(SpielState::SpielEnde);
                                        break;
                                    case 2:
                                        //Beim Tic Tac Toe kann ebenfalls immer nur der aktive Spieler gewinnen
                                        //dieser Case ist nur der Vollstaendigkeit halber vorhanden
                                        break;

                                    default:
                                        //Ansonsten geht das Spiel mit der naechsten Runde weiter
                                        enterState(SpielState::ComputerMachtTTTZug);
                                        break;
                                }
                        }
                    break;
                            ...
                            switch (spielLogik.spielendePruefenTTT(menschSpieler, computerSpieler))
                                {
                                    ...
                                    case 1:
                                        ...
                                        break;
                                    case 2:
                                        //Wurden 3 O in einer Reihe/Spalte/Diagonale platziert gewinnt der Computer und das Spiel endet
                                        spielfeldanzeigeAktualisierenTTT();
                                        spielDarstellung.gameOverMeldungAnzeigen();
                                        enterState(SpielState::SpielEnde);
                                        break;
...
void Spiel::spielfeldanzeigeAktualisierenTTT()
    {
        spielDarstellung.anzeigeBereinigen();
        spielDarstellung.ueberschriftTTT();
        //Hier genuegt das Zeichnen eines Spielfelds, da dieses fuer beide Spieler bekannt und identisch ist
        spielDarstellung.spielfeldZeichnen(menschSpieler.getSpielfeld());
    }
